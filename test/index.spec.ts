...
describe('extend() priority extender', () => {
  const obj = {
    method1() {
      return 3;
    },
  ...
  it('should call methods of the extender even when there is a method with the same name in it', () => {
    const result = extendedObj.method1();
  ...
  it('should assign a value for the real object, not the proxy, when the property primary does not exist in it', () => {
    ...
    expect((obj as any).property).to.be.eq('value2');
    expect('property' in obj).to.be.true;
  });
